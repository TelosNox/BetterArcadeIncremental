...
**Zuletzt abgeschlossen:** Phase 7p (Boost Barrage Bewegungs-/Kampf-Animation — vierte Playtest-Korrektur, game-spec.md 4.4 "Bewegungs-/Kampf-Animation", an Space Invaders/Galaga/Galaxian orientiert) — implementiert, alle automatisierten Checks grün (391/391 Tests unveraendert, reine Darstellungsschicht ohne Engine-/Testaenderung), zusaetzlich per Playwright-Skript mit Mehrfach-Screenshots WAEHREND einer laufenden Welle visuell verifiziert (siehe Abschnitt unten), aber noch nicht vom Nutzer gespielt.
**Läuft/als Nächstes:** Nutzer-Playtest für Phase 7p (und weiterhin fuer die zugrundeliegende Mechanik aus 7m–7o) abwarten, außerdem weiterhin ausstehender Playtest für 7j/7k/7l (Trap Tunnels' neues Kernmodell). Danach voraussichtlich Champion's Ledger als letztes Genre-Rework-Experiment (einziger noch verbleibender Automat im alten gemeinsamen Zyklus-Modell), oder Balance-/Politur-Arbeit — noch nicht entschieden. Bekannte, weiterhin nicht behobene Punkte: Progression/Balance-Tuning bleibt zurückgestellt; Phase 8 (Politur) bleibt zurückgestellt. Bekannte kleine kosmetische Einschraenkung aus 7p: der Formations-Bounce nutzt eine FESTE seitliche Amplitude unabhaengig von der Restgegnerzahl (echtes Space Invaders leitet die Bounce-Breite aus der Bounding-Box der noch lebenden Gegner ab) — bei nur noch 1–2 verbliebenen Gegnern kann der einzelne Sprite dadurch sichtbar ueber einen Bunker hinwegschwingen; rein optisch, keine Funktionsauswirkung.

## NEUE PHASE 7p: Boost Barrage Bewegungs-/Kampf-Animation (2026-07-11, vierte Playtest-Korrektur, Space-Invaders/Galaga/Galaxian-Vorbild)

**Problem (vierter Playtest):** Phase 7o (Sprite-Set) verbesserte die Silhouetten, aber die Bewegungsebene blieb ein einfacher Laser-/Gegenfeuer-Flug auf einer ansonsten ruhenden/statischen Formation (nur eine Markierungsbox um den "aktuellen" Gegner) — das eigentliche "Space Shooter"-Feeling fehlte weiterhin. Auf Nutzerwunsch wurde konkret an Space Invaders/Galaga/Galaxian recherchiert statt an abstrakten Animations-Haekchen; game-spec.md 4.4 "Bewegungs-/Kampf-Animation" wurde komplett neu gefasst (ersetzt die vorherige Fassung vollstaendig) und benennt vier konkrete, uebernommene Signatur-Elemente. Weiterhin: keine Spieler-Steuerung, `BoostBarrageEngine.ts` unangetastet.

**Umsetzung, ausschliesslich in `src/game/scenes/BoostBarrageScene.ts`:**

1. **Formation als starrer, gemeinsam bewegter Block (Space-Invaders-Signatur):** Neuer `update(time, delta)`-Lifecycle-Hook bewegt `this.formationOffsetX/Y` kontinuierlich (Bounce zwischen ±90px, an jedem Bildschirmrand-Abprallen Richtungswechsel + `FORMATION_STEP_DOWN`-Schritt naeher zum Schiff, bis `FORMATION_MAX_DESCENT`). Tempo = `FORMATION_BASE_SPEED + engine.getDestroyedCount() * FORMATION_SPEED_PER_DESTROYED` -- direkte visuelle Kopplung an die bereits vorhandene Engine-Eskalation, keine neue Engine-Logik.
2. **Einflug-Choreografie bei Wellenbeginn (Galaga-Signatur):** Neue Methode `spawnWaveIntro()` (aufgerufen von `beginWave()`, das jetzt VOR jeder Welle steht -- ersetzt sowohl den alten Wellenwechsel-Zweig in `runLoop()` als auch den direkten `renderPhase()+runLoop()`-Aufruf in `startNewRun()`): Gegner spawnen abwechselnd links/rechts ausserhalb des Bildschirms, fliegen ueber einen Zwischenpunkt in ihre Formations-Slots (zweistufiger Tween, `Sine.easeIn` dann `Back.easeOut` fuer ein sichtbares "Einschwingen"), zeitlich leicht pro Gegner versetzt. Bounce-Bewegung startet erst, wenn alle eingeflogen sind.
3. **Sturzangriff (Galaga/Galaxian-Signatur):** Neue Methode `runDiveAttack()` ersetzt die alte `playCombatEffect()` vollstaendig: der aufgeloeste Gegner wird aus der Formation geloest (Status `diving`, dadurch von der Bounce-Bewegung ausgenommen), fliegt in einer Schwungbahn (`Cubic.easeIn`) zu einem Angriffspunkt nahe dem Schiff, dort findet der Laser-/Gegenfeuer-Austausch statt (`fireShotLine`/`spawnExplosion`/`spawnShipHitFlash`/`spawnEscapeRing`, technisch unveraendert aus Phase 7o uebernommen). Bei Zerstoerung: Explosion, Sprite wird entfernt. Bei Ueberleben: Ruecksturz-Tween (`Cubic.easeOut`) in den AKTUELLEN (durch den Bounce inzwischen weitergewanderten) Formations-Slot, danach wieder Teil der Bounce-Bewegung (gedimmt, wie zuvor).
4. **Erodierende Bunker-Bloecke (optional, umgesetzt):** Drei Bunker aus je 6 kleinen Rechtecken zwischen Sturzangriff-Zielpunkt und Schiff. Jeder Bomber-Treffer der laufenden Welle (`applyEncounterResult()`) entfernt per `registerBomberHit()` einen zufaelligen verbliebenen Block -- rein dekorative Ableitung aus bereits vorhandenen Gefechtsdaten, kein neuer Engine-Zustand. Bunker werden bei jedem Wellenstart (`resetBunkers()`) vollstaendig zurueckgesetzt.

**Architektur-Konsequenz (wichtig fuer Korrektheit, nicht nur Kosmetik):** Die Formations-/Kampf-Schicht (`formationSlots`, Marker, Bomber-Blink-Tween, Bunker) wurde bewusst KOMPLETT von `dynamicObjects`/`clearDynamic()` GETRENNT. Vorher wurden diese Objekte bei JEDEM `renderPhase()`-Aufruf neu gezeichnet, was bei jedem Boost-Button-Klick passiert -- waere die neue, laenger laufende Sturzangriff-Animation weiterhin darueber gekoppelt gewesen, haette ein Boost-Klick MITTEN im Sturzangriff den fliegenden Gegner-Sprite zerstoert und den `onComplete`-Callback nie ausgeloest (Softlock, `runLoop()` haette nie weitergemacht). Loesung: `dynamicObjects` ist jetzt AUSSCHLIESSLICH fuer UI-Chrome (Boost-Buttons/Upgrade-Shop/Zur-Halle-Button) zustaendig; alle Ein-mal-Kampf-Effekte (Laser/Explosion/Trefferblitz/Ausweich-Ring) loeschen sich seither ueber ihren eigenen Tween-`onComplete` SELBST, ohne externe Aufraeum-Liste. `dynamicTweens` (vorher fuer Bomber-Blink + Kampf-Effekte) wurde dadurch komplett ueberfluessig und entfernt.

**Verifiziert:** `npm test` (**391/391 gruen**, unveraendert), `npm run lint` sauber, `npx tsc --noEmit` sauber, `npm run build-nolog` erfolgreich. Zusaetzlich per Playwright-Skript gegen `npm run dev-nolog` (temporaere Playwright-Installation danach wieder vollstaendig entfernt, `npm uninstall --no-save playwright`, kein Diff in package.json/package-lock.json) mit MEHREREN Screenshot-Serien WAEHREND einer laufenden Welle verifiziert (nicht nur Vorher/Nachher): (1) Einflug-Serie zeigt Gegner an unterschiedlichen Zwischenpositionen von ausserhalb des Bildschirms kommend, bevor sie in die 3-Spalten-Formation einrasten; (2) Bounce-Serie (10 Screenshots im 250ms-Takt) zeigt sichtbar wechselnde X-Positionen und zunehmenden Y-Versatz derselben Formation ueber die Zeit; (3) dichte Sturzangriff-Serie (90 Screenshots im 45ms-Takt) faengt den Moment ein, in dem sich ein Ziel-Gegner sichtbar aus der Formation loest und zu einem Angriffspunkt direkt ueber dem Schiff fliegt, sowie den erfolgreichen Rueckflug eines ueberlebenden Gegners in seinen (bereits weitergewanderten) Formations-Slot, danach korrekt gedimmt und wieder Teil der Bounce-Bewegung. Bunker-Erosion bestaetigt (sichtbar fehlende Bloecke nach Bomber-Treffern in spaeteren Frames). Keine Konsolenfehler/`pageerror`-Eintraege ueber den gesamten Testlauf. **Noch nicht vom Nutzer selbst gespielt/bestaetigt** -- das ist der naechste Schritt, kein automatisierter Ersatz dafuer.

## Phase 7o: Boost Barrage Sprite-/Textur-Set (2026-07-11, zweite Playtest-Korrektur zu Phase 7n)

**Problem (zweiter Playtest):** Phase 7n (Formation-Raster, Sternfeld, Schiff-Dreieck, Schuss-/Explosions-Primitive) reichte laut Nutzer-Playtest IMMER NOCH NICHT, um "Weltraum-Shooter" statt "Icon-Liste mit Effekten" zu vermitteln. Mit dem Nutzer abgestimmte Entscheidung: eine bewusste, eng auf Automat 3 begrenzte Ausnahme von der Primitive-Only-Regel (CLAUDE.md "Ausnahme Automat 3", game-spec.md 4.4 "Visuelle Umsetzung", beide Dokumente entsprechend ergaenzt/ueberarbeitet). Mechanik/Engine (`BoostBarrageEngine.ts`) bleiben unangetastet — der zunaechst erwogene "zwingender Korrekt-Konter pro Gegnertyp"-Ansatz wurde erneut verworfen (zu grosse strukturelle Naehe zum fuer Champion's Ledger geplanten Stance-Konter-Modell).

**Umsetzung, ausschliesslich in `src/game/scenes/BoostBarrageScene.ts`:**

1. **Sechs zur Laufzeit gebackene Texturen** (`ensureTextures()`, einmalig pro Game-Instanz ueber `textures.exists`-Guard): Spielerschiff (`bb-ship`), drei Gegner-Silhouetten (`bb-scout` schlank/klein, `bb-bomber` boxig/breit, `bb-elite` angular mit Pfeilform+Heckspitze), Schuss-/Laser-Effekt (`bb-laser`, neutral weiss, drei ueberlagerte Glow-Ellipsen), Explosion (`bb-explosion`, Glow-Kreis + gezackter Burst + heller Kern). Technik-Entscheidung (CLAUDE.md ueberlaesst das Claude Code): `this.make.graphics(...).generateTexture(...)` aus Polygon-Punktlisten zur Laufzeit statt mitgelieferter SVG-Dateien — bleibt vollstaendig im versionierten TypeScript-Code, kein neuer Binaer-Asset-Ladepfad/Fehlerfall. Alle Punktlisten sind bewusst so gewaehlt, dass ihre Bounding-Box exakt auf der Canvas-Mitte zentriert ist, damit `setOrigin(0.5)` das Sprite exakt auf der (weiterhin aus Phase 7n uebernommenen) Formations-/Ziel-Position platziert.
2. **Gegner-Silhouetten ersetzen Kreis/Quadrat/Dreieck:** `drawEnemyIcon()` nutzt jetzt `this.add.image(...).setTint(getEnemyTypeColor(type))` auf einer neutral weissen Silhouette (schwarzer Rand bleibt durch multiplikatives Tinting immer schwarz). Barrierefreiheits-Grundsatz (CLAUDE.md) bleibt gewahrt: Farbe UND Form (jetzt distinktere Schiff-Silhouetten statt geometrischer Primitive) UND Buchstabe (`S`/`B`/`E`-Textoverlay unveraendert). Legende (`renderLegend()`) verwendet dieselben getinteten Sprites statt der alten Primitive, damit Legende und Formation optisch konsistent sind.
3. **Schuss-/Laser-Effekt:** `fireShotLine()` ersetzt die duenne `Phaser.GameObjects.Line` durch einen `bb-laser`-Sprite, der auf die tatsaechliche Schuss-Distanz gestreckt (`setDisplaySize`) und um den Schusswinkel rotiert wird (`Phaser.Math.Angle.Between`/`Distance.Between`) — pro Gefechts-Ausgang weiterhin unterschiedlich eingefaerbt (Gold=zerstoert, Rot=Bomber trifft, Silber=ausgewichen), exakt wie zuvor.
4. **Explosions-Sprite:** `spawnExplosion()` ersetzt den weissen Blitz-Kreis durch den `bb-explosion`-Sprite (skaliert 0.5→1.5 mit Alpha-Fade), der expandierende Gold-Ring bleibt als zusaetzliches Formmerkmal erhalten (Explosion=gefuellter Burst, Ausweich-Ring=reiner Ring — weiterhin unterscheidbar).
5. **Bewusst unveraendert:** Formation-Raster/`formationPosition()`, Sternfeld, Bomber-Blink-Warnung, `spawnShipHitFlash()`/`spawnEscapeRing()` (bleiben absichtlich einfache Primitive, da sie bereits klar unterscheidbar waren und nicht Teil des von CLAUDE.md benannten Mindest-Sprite-Sets sind), alle drei Upgrade-Achsen, Blind-Erwartungswert-Garantie.

**Verifiziert:** `npm test` (**391/391 gruen**, unveraendert), `npm run lint` sauber, `npx tsc --noEmit` sauber, `npm run build-nolog` erfolgreich. Zusaetzlich per Playwright-Skript gegen `npm run dev-nolog` (Skript + temporaere Playwright-/Chromium-Installation danach wieder vollstaendig entfernt, `npm uninstall --no-save playwright`, kein Diff in package.json/package-lock.json, nicht Teil des Repos) verifiziert: Ein via localStorage vorbereiteter Spielstand (Automat 1 als abgeschlossen markiert, Automat 3 freigeschaltet) plus Klick durch die Hallen-UI fuehrt direkt in Boost Barrage; Screenshot zeigt Sternfeld, deutlich unterscheidbare Schiff-/Gegner-Silhouetten in 3-Spalten-Formation, farbige Legende mit denselben Sprites. Eine schnelle Screenshot-Serie (40ms-Takt, 80 Frames) faengt sowohl den "Bomber trifft"-Fall (roter Laser-Bolt vom Bomber zum Schiff + rotes Einschlag-Flackern) als auch den "zerstoert"-Fall (goldener Laser-Bolt vom Schiff zum Ziel + Explosions-Sprite + expandierender Ring, exakt auf der sichtbaren Roster-Position) live in Aktion ein. Keine Konsolenfehler/`pageerror`-Eintraege ueber den gesamten Testlauf. **Noch nicht vom Nutzer selbst gespielt/bestaetigt** -- das ist der naechste Schritt, kein automatisierter Ersatz dafuer.

## Phase 7n: Boost Barrage Visuelle Umsetzung (2026-07-11, Korrektur nach Playtest-Screenshot zu Phase 7m)

**Problem (Nutzer-Feedback anhand eines Screenshots):** Phase 7m erfuellte die Mechanik korrekt, wirkte aber optisch nicht wie ein Weltraum-Shooter — eine flache horizontale Icon-Reihe mit Textfeedback darunter, ohne Spielerschiff, Anflugrichtung, Kampf-Feedback oder Weltraum-Kulisse; haette optisch genauso gut eine beliebige Icon-Liste sein koennen. `docs/game-spec.md` 4.4 wurde um den Absatz "Visuelle Umsetzung (Korrektur nach Playtest-Screenshot, 2026-07-11, verbindlich)" ergaenzt (vier Punkte). Ausdruecklich NUR die Darstellungsschicht betroffen -- `BoostBarrageEngine.ts`, alle Tests, die Blind-Erwartungswert-Garantie und die drei Upgrade-Achsen bleiben unangetastet (391/391 Tests unveraendert gruen).

**Umsetzung, ausschliesslich in `src/game/scenes/BoostBarrageScene.ts`, weiterhin nur Phaser-Grafik-Primitive (CLAUDE.md: kein Asset-Bedarf bis Phase 8):**

1. **Spielerschiff:** Statisches Dreieck (`renderShip()`, `SHIP_POSITION` unten in derselben Formations-Spalte) mit heller Fuellung + cyanem Rand, einmalig in `create()` gerendert (kein `dynamicObjects`-Teil, bleibt ueber die gesamte Szenen-Lebensdauer bestehen).
2. **Formation statt Liste:** `rosterCenter()` (flache Reihe) ersetzt durch `formationPosition(index, count)` -- Space-Invaders-artiges Raster (max. 3 Spalten, mehrere Zeilen), reine Funktion OHNE Zeitabhaengigkeit, damit Roster-Rendering UND Kampf-Effekt-Zielung garantiert exakt uebereinstimmen.
3. **Kampf-Feedback pro Gefecht:** Neue Methode `playCombatEffect()`, aufgerufen von `runLoop()` direkt nach jedem `resolveNextEncounter()`. Drei visuell unterscheidbare Faelle (Farbe UND Form/Ort unterschiedlich, nicht nur Farbe): "zerstoert" (Schuss-Linie Schiff→Ziel + gefuellter Blitz + expandierender Ring am Ziel), "Bomber trifft" (Schuss-Linie Ziel→Schiff + roter Einschlag-Burst + kurzes Anflackern des Schiffs selbst -- Schiff bleibt bestehen), "entkommt/ausgewichen" (Schuss-Linie Schiff→Ziel + reiner kuehler Ring OHNE Fuellung am Ziel, bewusst unterscheidbar von der Explosion). Alle Effekt-Objekte/-Tweens laufen ueber `dynamicObjects`/`dynamicTweens` und werden von der naechsten `renderPhase()`-`clearDynamic()` automatisch entfernt; Flacker-Tween-Dauer am Schiff bewusst deutlich unter `WAVE_TRANSITION_MS` gehalten, damit sie sicher durchlaeuft, bevor eine Wellenuebergangs-`clearDynamic()` sie stoppen koennte.
4. **Weltraum-Kulisse:** `renderStarfield()` erzeugt einmalig 80 zufaellig positionierte, unterschiedlich helle Kreise (`this.add.circle`) als Hintergrund, ganz zu Beginn von `create()` (vor allen anderen Elementen) gerendert.

**Bewusst NICHT umgesetzt:** kein Zeit-/Positions-abhaengiger "Anflug"-Effekt (Formation wird NICHT graduell groesser/naeher animiert) -- waere zusaetzliche Komplexitaet mit Desync-Risiko zwischen Roster-Rendering und Kampf-Effekt-Zielung gewesen, game-spec.md 4.4 verlangt Formation ODER Anflugbewegung, nicht beides; die Grid-Formation allein loest das gemeldete "sieht aus wie eine Liste"-Problem bereits vollstaendig.

**Verifiziert:** `npm test` (**391/391 gruen**, unveraendert -- reine Darstellungsschicht, keine Engine-/Datenaenderung), `npm run lint` sauber, `npx tsc --noEmit` sauber, `npm run build-nolog` erfolgreich. Zusaetzlich per Playwright-Skript gegen `npm run dev-nolog` (Skript + temporaere Playwright-Installation danach wieder vollstaendig entfernt, nicht Teil des Repos/package.json) mit einem echten Vorher/Nachher-Screenshot-Vergleich verifiziert (Phase-7m-Stand per `git stash` auf denselben laufenden Dev-Server zurueckgesetzt, Screenshot genommen, dann `git stash pop` und erneuter Screenshot -- kein bloss beschriebener, sondern tatsaechlich gegenuebergestellter Vergleich): Vorher zeigt exakt das gemeldete Problem (flache Reihe, kein Schiff, kein Sternfeld, keine Kampf-Effekte). Nachher zeigt Sternfeld, Schiff-Dreieck und 3-Spalten-Formation von Beginn an sichtbar; ein per schneller Screenshot-Serie (60ms-Takt) eingefangener Frame zeigt den Schuss-Effekt live in Aktion (goldene Linie vom Schiff zum soeben zerstoerten Scout, expandierender gelber Ring am Einschlagspunkt), ein Folgeframe zeigt die Linie beim Ausblenden -- Zielposition des Effekts stimmt exakt mit der sichtbaren (bereits abgedunkelten) Roster-Ikone ueberein, kein Versatz. Keine Konsolenfehler ueber den gesamten Testlauf. **Noch nicht vom Nutzer selbst gespielt/bestaetigt** -- das ist der naechste Schritt, kein automatisierter Ersatz dafuer.
