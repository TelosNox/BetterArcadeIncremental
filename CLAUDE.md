...
- Automaten dürfen eigene, genre-spezifische Darstellungs-/Interaktionslogik (eigene Phaser-Szene) bekommen, wenn ihre Mechanik strukturell vom gemeinsamen Zyklus-Modell abweicht (siehe `game-spec.md` 4.2 für Automat 1 als erstes Beispiel, Stand 2026-07-10). Die gemeinsame Buchhaltung (`EconomyStore`, `SaveSystem`, Tickets-/Meilenstein-Anbindung) bleibt trotzdem geteilt und darf nicht dupliziert werden. Automaten, die weiterhin dem gemeinsamen Zyklus-Modell aus `game-spec.md` 4.1/4.1b/4.1c folgen (aktuell nur noch Automat 4), bleiben über die bestehende generische `MachineScene.ts`
- Phasenreihenfolge aus `implementation-plan.md` Abschnitt 3 einhalten; jede Phase hat eigene Abnahmekriterien
- Es existieren noch keine Grafik-/Sound-Assets. Bis einschließlich Phase 8 (Politur) Platzhalter verwenden (Phaser Graphics-Primitives: Rechtecke, Kreise, Text) statt Bild- oder Audio-Dateipfade zu erfinden. **Ausnahme Automat 3 ("Boost Barrage", ab Phase 7n, 2026-07-11):** Playtest zeigte, dass ein Weltraum-Shooter ohne erkennbare Schiff-/Gegner-/Effekt-Silhouette sein Genre-Versprechen nicht einlöst — reine Primitive reichten nicht, um "Shooter" statt "Icon-Liste" zu vermitteln (siehe `game-spec.md` 4.4 "Visuelle Umsetzung"). Bewusste, eng begrenzte Ausnahme: ein kleines, selbst erzeugtes Sprite-/Textur-Set (Spielerschiff, Schuss-/Laser-Effekt, Explosion, drei Gegner-Silhouetten) ist NUR für diesen Automaten erlaubt — weiterhin keine extern eingekauften/lizenzierten Assets, genaue Technik (z. B. zur Laufzeit generierte Canvas-/Graphics-Texturen vs. mitgelieferte SVG-Dateien) im Ermessen von Claude Code. Automaten 1, 2 und 4 bleiben bis einschließlich Phase 8 auf reinen Phaser-Grafik-Primitiven — diese Ausnahme überträgt sich NICHT automatisch auf andere Automaten, dafür erneut Rücksprache halten.
